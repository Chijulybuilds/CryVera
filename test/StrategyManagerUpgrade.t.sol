...
import {
    StrategyManagerUpgradeable
} from "../src/core/StrategyManagerUpgradeable.sol";
...
contract StrategyManagerUpgradeTest is Test {
    ...
    function testStrategyManagerUpgradePreservesState() public {
        ...
        bytes memory initData = abi.encodeWithSelector(
            StrategyManagerUpgradeable.initialize.selector,
            deployer
        );
        ...
        StrategyManagerUpgradeable proxied = StrategyManagerUpgradeable(
            address(proxy)
        );
        ...
        // state preserved
        assertEq(
            StrategyManagerUpgradeable(address(proxy)).vault(),
            address(0xBEEF)
        );
    }
}
