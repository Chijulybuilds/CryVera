...
import {
    OracleManagerUpgradeable
} from "../src/core/OracleManagerUpgradeable.sol";
...
contract OracleManagerUpgradeTest is Test {
    ...
    function testOracleManagerUpgradePreservesRegistry() public {
        ...
        bytes memory initData = abi.encodeWithSelector(
            OracleManagerUpgradeable.initialize.selector,
            deployer,
            address(registry)
        );
        ...
        OracleManagerUpgradeable proxied = OracleManagerUpgradeable(
            address(proxy)
        );
        ...
        assertEq(
            address(OracleManagerUpgradeable(address(proxy)).i_registry()),
            address(registry)
        );
    }
}
