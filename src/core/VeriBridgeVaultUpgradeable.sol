...
contract VeriBridgeVaultUpgradeable is
    Initializable,
    AccessControl,
    ReentrancyGuard
{
    ...
    function deposit(
        ...
        address receiver,
        uint256 strategyId
    ) external nonReentrant whenDepositsOpen returns (uint256 shares) {
    ...
    function redeem(
        ...
        assets = Math.mulDiv(
            value,
            10 ** assetRegistry.getAsset(asset).decimals,
    ...
    function harvest(uint256 strategyId) external nonReentrant {
        address strategy = strategyManager.getStrategyAddress(strategyId);
        (uint256 beforeAssets, uint256 afterAssets) = strategyManager
    ...
    function totalAssets() public view returns (uint256 value) {
        ...
        for (uint256 i; i < count; ++i) {
            ...
            value += _value(
                strategyManager.strategyAsset(strategy),
    ...
    function _ensureLiquidity(address asset, uint256 needed) private {
        ...
        for (uint256 i; i < count && remaining != 0; ++i) {
            ...
            uint256 got = strategyManager.withdrawFromStrategy(
                strategy,
                request,
                address(this)
            );
    ...
    function _value(
        ...
        return
            Math.mulDiv(amount, oracle.getPrice(asset), 10 ** config.decimals);
    }
}
