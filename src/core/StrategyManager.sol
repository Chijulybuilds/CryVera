...
/// @notice Permissioned router for vault capital. It deliberately keeps no user or strategy-value accounting.
contract StrategyManager is AccessControl, ReentrancyGuard {
    ...
    /// @notice One-time wiring step that resolves the vault/manager deployment cycle.
    ...
    function registerStrategy(address strategy, uint256 cap)
        external
        onlyRole(STRATEGY_ADMIN_ROLE)
        returns (uint256 id)
    {
        if (strategy == address(0)) revert Errors.ZeroAddress();
        if (strategyId[strategy] != 0) revert Errors.StrategyAlreadyRegistered();
        // Verifies the adapter exposes a real supported asset before registration.
        if (address(IStrategy(strategy).asset()) == address(0)) revert Errors.InvalidStrategy();
        id = s_nextId++;
        ...
        returns (uint256 received)
    {
        ...
        if (pulled != assets) revert Errors.InsufficientReceived(assets, pulled);
        token.forceApprove(strategy, pulled);
        received = IStrategy(strategy).deposit(pulled);
        token.forceApprove(strategy, 0);
        if (received == 0 || received > pulled) revert Errors.StrategyDepositFailed();
    }
    ...
    function harvestStrategy(address strategy)
        external
        onlyVault
        nonReentrant
